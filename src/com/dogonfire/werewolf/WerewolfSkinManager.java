...
public class WerewolfSkinManager
{
	...
	public int setWerewolfSkin(Player player)
	{
		...
		Packet spawnPacket = (Packet) skin.getPlayerSpawnPacket(player.getLocation(), (short) player.getItemInHand().getTypeId());
	...
	public void removeSkinFromWorld(org.bukkit.World world, Player player)
	{
		if (!this.skins.containsKey(player.getUniqueId()))
		{
			return;
		...
		for (Player observer : ((org.bukkit.World) world).getPlayers())
		{
	...
	public void sendWorldChange(Player player, World fromWorld)
	{
		...
		Packet revivePlayerPacket = (Packet) skin.getPlayerSpawnPacket(player.getLocation(), (short) player.getItemInHand().getTypeId());
		Packet reviveListPacket = skin.getPlayerInfoPacket(player, true);
		
		if (killListPacket == null)
		{
			disguiseToWorld((org.bukkit.World) fromWorld, player, new Packet[] { killPacket });
		}
		else
		{
			undisguiseToWorld((org.bukkit.World) fromWorld, player, new Packet[] { killPacket, killListPacket });
		}
		disguiseToWorld(player.getWorld(), player, new Packet[] { reviveListPacket, revivePlayerPacket });
	}

	public void sendPacketsToWorld(org.bukkit.World world, Packet[] packets)
	{
	...
	public void disguiseToWorld(org.bukkit.World world, Player player, Packet[] packets)
	{
		for (Player observer : world.getPlayers())
		{
	...
	public void undisguiseToWorld(org.bukkit.World world, Player player, Packet[] packets)
	{
		for (Player observer : ((org.bukkit.World) world).getPlayers())
		{
